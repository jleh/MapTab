      ...
      <div className='modal pad4 fill-white keyline-all round' id='settings'>
        ...
        <form id='form-settings' onSubmit={this.handleSubmit}>
          ...
          <fieldset  name='map'>
            ...
            <div className='radio-pill pill clearfix col12' name='map'>
              {this.state.mapSettings.map(item => {
                return <span key={item.id}><input type='radio' name='map' value={item.id} id={item.id} checked={item.checked} onChange={this.handleChangeMap.bind(this, item.id)} /><label htmlFor={item.id} className='col4 button icon check'>{item.name}</label></span>
              })}
            </div>
          </fieldset>
          ...
          <span className='fr micro quiet pad1y icon mail'>
            ...
            <a href='https://github.com/jleh/MapTab' target='_blank' className='icon github quiet'>Fork</a>
          </span>
        </form>
      </div>
    );
